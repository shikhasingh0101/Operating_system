...
int main() {
    ...
    while (1) {
        ...
        if (sendto(client_socket, buffer, strlen(buffer), 0,(const struct sockaddr *)&server_address, sizeof(server_address)) == -1) {
        ...
        ssize_t bytes_received = recvfrom(client_socket, buffer, MAX_BUFFER_SIZE, 0,(struct sockaddr *)&server_echo_address, &server_echo_address_len);
        if (bytes_received == -1) {
